...
<div class="mx-auto max-w-7xl space-y-5">
    ...
    <form method="GET" action="{{ route('enduser.projects.index') }}" class="flex flex-wrap items-center gap-3">
        ...
        <button type="submit"
            class="h-10 rounded-xl border border-indigo-200 bg-indigo-50 px-4 text-xs font-semibold text-indigo-600 transition hover:bg-indigo-100">
            {{ __('app.search') }}
        </button>
        <a href="{{ route('enduser.projects.index') }}"
            class="h-10 inline-flex items-center rounded-xl border border-red-200 bg-red-50 px-4 text-xs font-semibold text-red-500 transition hover:bg-red-100">
            {{ __('app.clear') }}
        </a>
    </form>
