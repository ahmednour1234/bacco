    ...
    '@graph'   => [
        [
            ...
            'itemListElement' => [
                ...
                ['@type'=>'ListItem','position'=>2,'name'=>$isAr ? '????? ????? ??????' : 'News','item'=>'https://www.qimta.com/news'],
            ],
        ],
        [
            '@type'      => 'WebPage',
    ...
    <div style="background:#fff; border-bottom:1px solid #eee; padding:10px 0;">
        <div style="max-width:1200px; margin:0 auto; padding:0 24px;">
            <nav style="font-size:13px; color:#888; display:flex; align-items:center; gap:6px;">
                <a href="{{ url('/') }}" style="color:#888; text-decoration:none;">{{ $isAr ? '????????' : 'Home' }}</a>
                ...
                <span style="color:#333;">{{ $isAr ? '???????' : 'News' }}</span>
            </nav>
        </div>
    </div>
                ...
                <form method="GET" action="{{ route('news') }}" style="display:flex; align-items:center;">
                    ...
                    <div style="position:relative; width:280px;">
                        ...
                        <input type="search" name="search" value="{{ $search }}"
                               placeholder="{{ $isAr ? '???? ?? ????????…' : 'Search articles, videos, guides…' }}"
    ...
    <div style="max-width:1200px; margin:0 auto; padding:40px 24px;">
        ...
        @if(!$featured && $rest->isEmpty())
            <div style="text-align:center; padding:80px 0; color:#999;">
                ...
                <p style="font-size:16px;">{{ $isAr ? '?? ???? ??????.' : 'No articles found.' }}</p>
            </div>
        @else
