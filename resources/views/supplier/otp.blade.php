...
@section('title', 'Verify Code – Supplier – Qimta')
...
@section('form')
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 text-sm mt-1.5">
            Code sent to
            <span class="font-semibold text-slate-700">{{ session('supplier_otp_email') ?? 'your email' }}</span>
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('supplier.otp.verify') }}"
    ...
    <form id="resendForm" method="POST"
          action="{{ route('supplier.forgot-password.send') }}" class="hidden">
        @csrf
        <input type="hidden" name="email" value="{{ session('supplier_otp_email') }}">
