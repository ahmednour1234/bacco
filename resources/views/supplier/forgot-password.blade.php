...
@section('title', 'Forgot Password – Supplier – Qimta')

@section('left-heading', 'Reset your password securely.')
@section('left-subtext', 'We\'ll send a one-time code to your registered email address.')

@section('form')
    <div class="mb-7">
        <a href="{{ route('supplier.login') }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 hover:text-slate-700 transition mb-4">
            <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
            Back to Sign In
        </a>
        <h2 class="text-2xl sm:text-3xl font-bold text-slate-900">Forgot Password</h2>
    ...
    <form method="POST" action="{{ route('supplier.forgot-password.send') }}" class="space-y-4">
        @csrf
