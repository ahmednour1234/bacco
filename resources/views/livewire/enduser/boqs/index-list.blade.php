    ...
    @else
        ...
        <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav class="flex items-center gap-1">
                ...
                @else
                    <button wire:click="$set('page', {{ $boqs->currentPage() - 1 }})" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-50">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                        </svg>
                    </button>
                ...
                @foreach($boqs->getUrlRange(max(1, $boqs->currentPage() - 2), min($boqs->lastPage(), $boqs->currentPage() + 2)) as $page => $url)
                    ...
                    @else
                        <button wire:click="$set('page', {{ $page }})" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-sm text-slate-600 transition hover:bg-slate-50">{{ $page }}</button>
                    @endif
                @endforeach
