                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead><tr class="border-b border-slate-100 bg-slate-50">
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wide text-slate-500 w-32">{{ app()->getLocale()==='ar' ? 'التحقق' : 'Verification' }}</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @foreach($pricedItems as $pi)
                                <tr class="hover:bg-slate-50/60">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        @php
                                            $verdict = $pi['price_verdict'] ?? null;
                                            $isAr = app()->getLocale()==='ar';
                                            $vMap = [
                                                'confirmed' => ['bg-emerald-50 text-emerald-700', $isAr ? 'مؤكّد' : 'Confirmed'],
                                                'corrected' => ['bg-amber-50 text-amber-700',     $isAr ? 'مُصحّح' : 'Corrected'],
                                                'flagged'   => ['bg-red-50 text-red-600',         $isAr ? 'مراجعة' : 'Review'],
                                            ];
                                        @endphp
                                        @if($verdict && isset($vMap[$verdict]))
                                            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $vMap[$verdict][0] }}"
                                                  @if(!empty($pi['verification_note'])) title="{{ $pi['verification_note'] }}" @endif>
                                                {{ $vMap[$verdict][1] }}
                                            </span>
                                        @else
                                            <span class="text-slate-300">—</span>
                        ...
                        <tfoot>
                            <tr class="border-t-2 border-slate-200 bg-slate-50"><td colspan="6" class="px-4 py-3 text-end text-sm font-semibold text-slate-600">{{ app()->getLocale()==='ar'?'المجموع':'Subtotal' }}</td><td class="px-4 py-3 text-end font-bold text-slate-800">{{ number_format($quotationTotal, 2) }} SAR</td></tr>
                            <tr class="bg-slate-50"><td colspan="6" class="px-4 py-2 text-end text-xs text-slate-500">{{ app()->getLocale()==='ar'?'ضريبة القيمة المضافة (15%)':'VAT (15%)' }}</td><td class="px-4 py-2 text-end text-sm text-slate-600">{{ number_format($quotationTotal * 0.15, 2) }} SAR</td></tr>
                            <tr class="bg-emerald-50"><td colspan="6" class="px-4 py-3 text-end text-sm font-bold text-emerald-700">{{ app()->getLocale()==='ar'?'الإجمالي شامل الضريبة':'Grand Total incl. VAT' }}</td><td class="px-4 py-3 text-end text-lg font-bold text-emerald-700">{{ number_format($quotationTotal * 1.15, 2) }} SAR</td></tr>
                        </tfoot>
                    </table>
                </div>
            </div>
