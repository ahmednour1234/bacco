...
>
    ...
    <div class="mb-8">
        <div class="flex items-center justify-center gap-4">
            <template x-for="s in [1,2,3,4]" :key="s">
                <div class="flex flex-col items-center">
                    ...
                    <div class="mt-2 text-xs font-semibold" :class="step >= s ? 'text-emerald-600' : 'text-slate-400'"
        ...
        <div class="relative h-2 mt-4 bg-slate-100 rounded-full overflow-hidden">
            <div class="absolute top-0 left-0 h-2 bg-emerald-500 transition-all duration-300" :style="'width: calc(' + ((step-1)/3) + ' * 100%)'"></div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        {{-- المرحلة الأولى: رفع الأصناف/مراجعة الأصناف --}}
        <div x-show="step === 1">
            {{-- معلومات المشروع (يمكنك نقلها للخطوة 2 لو أردت) --}}
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                <div class="p-6 space-y-6">
                    {{-- ...existing code for upload and manual items... --}}
                    @include('livewire.enduser.quotations.partials.boq-upload-and-items')
                </div>
            </div>
            <div class="flex justify-end mt-6">
                <button type="button" @click="step = 2" class="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700">
                    {{ __('app.next_step') }}
