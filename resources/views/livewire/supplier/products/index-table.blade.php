    ...
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('app.search_products') }}"
            ...
            <table class="min-w-full divide-y divide-slate-100">
                <thead>
                    <tr class="bg-slate-50">
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.product') }}</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.price') }}</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.lead_time') }}</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.min_qty') }}</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.status') }}</th>
                        <th class="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">{{ __('app.added') }}</th>
                    ...
                    @forelse($supplierProducts as $sp)
                        <tr class="group transition hover:bg-slate-50/60">
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-sm text-slate-600">
                                {{ $sp->lead_time_days ? $sp->lead_time_days . ' ' . __('app.days') : '—' }}
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                @if($sp->approval_status === 'pending')
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-1 text-xs font-semibold text-amber-700">
                                        <span class="h-1.5 w-1.5 rounded-full bg-amber-500"></span>{{ __('app.pending_approval') }}
                                    </span>
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <a href="{{ route('supplier.products.edit', $sp) }}" wire:navigate
                                        class="rounded-lg p-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700" title="{{ __('app.edit') }}">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <button wire:click="delete({{ $sp->id }})"
                                        ...
                                        class="rounded-lg p-1.5 text-red-400 transition hover:bg-red-50 hover:text-red-600" title="{{ __('app.remove') }}">
                                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-16 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-sm font-semibold text-slate-500">{{ __('app.no_products_catalogue') }}</p>
                                    <a href="{{ route('supplier.products.create') }}" wire:navigate
                                        class="text-xs text-emerald-600 underline hover:text-emerald-700">{{ __('app.add_first_product') }}</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
