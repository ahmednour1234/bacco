...
@section('title', __('catalog_import.title'))
...
<div class="p-6 max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-800">{{ __('catalog_import.title') }}</h1>
    </div>
    ...
    <form method="POST"
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ __('catalog_import.catalog_label') }}
                <span class="ml-1 text-xs font-normal text-gray-400">{{ __('catalog_import.catalog_hint') }}</span>
            </label>
            <select name="catalog_id"
                ...
                <option value="">{{ __('catalog_import.auto_create') }}</option>
        ...
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
                {{ __('catalog_import.files_label') }} <span class="text-red-500">*</span>
            </label>
            <div id="drop-zone"
                ...
                <p class="text-sm font-medium text-gray-600">{{ __('catalog_import.drop_zone') }}</p>
                <p class="mt-1 text-xs text-gray-400">{{ __('catalog_import.drop_hint') }}</p>
        ...
        <div class="rounded-lg bg-blue-50 border border-blue-100 text-blue-800 px-4 py-3 text-xs space-y-1">
            <p class="font-semibold">{{ __('catalog_import.expected_format') }}</p>
            <ul class="list-disc list-inside space-y-0.5">
                <li>{!! __('catalog_import.heading_row', ['row' => '<strong>' . __('catalog_import.row', ['number' => 4]) . '</strong>']) !!}</li>
                <li>{{ __('catalog_import.required_columns') }} <code>{{ implode('، ', __('catalog_import.columns')) }}</code></li>
                <li>{!! __('catalog_import.data_starts', ['row' => '<strong>' . __('catalog_import.row', ['number' => 5]) . '</strong>']) !!}</li>
            </ul>
        </div>
...
function showFileList(input) {
    ...
    if (hasOversized) {
        ...
        warn.textContent = @json(__('catalog_import.oversize_warning'));
        list.after(warn);
    } else {
        btn.disabled = false;
