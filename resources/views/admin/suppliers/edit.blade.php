...
@section('title', 'Edit Supplier – Qimta Admin')
...
@section('breadcrumb')
    ...
    <span class="text-xs text-slate-300">/</span>
    <a href="{{ route('admin.suppliers.show', $supplier->uuid) }}" wire:navigate class="text-xs font-medium text-slate-500 hover:text-slate-700 transition">{{ $supplier->name }}</a>
    <span class="text-xs text-slate-300">/</span>
...
@section('content')
<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.suppliers.update', $supplier->uuid) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="company_name" value="{{ old('company_name', $supplier->supplierProfile?->company_name) }}"
                        class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Division / Role in Company</label>
                    <input type="text" name="division" value="{{ old('division', $supplier->supplierProfile?->division) }}" placeholder="e.g. Sales Manager"
                        class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="address" rows="2"
                    class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100 resize-none">{{ old('address', $supplier->supplierProfile?->address) }}</textarea>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">City</label>
                    <input type="text" name="city" value="{{ old('city', $supplier->supplierProfile?->city) }}"
                        class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1.5">Country</label>
                    <input type="text" name="country" value="{{ old('country', $supplier->supplierProfile?->country) }}" placeholder="Saudi Arabia"
                        class="w-full rounded-xl border border-slate-200 px-3.5 py-2.5 text-sm text-slate-900 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100">
                </div>
            </div>
        </div>
